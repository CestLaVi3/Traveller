...
function activeSpot(pageNum){
	...
	$.post("../../../../activeController/findTeamByUserCreate.do",{"paramter":JSON.stringify(activeJoin)},showJoinActive,"json");
}
function showJoinActive(ret){
	...
	for(var i=0;i<ret.value.length;i++){
		...
		var a6=$("<a/>");
		a6.attr("href","../addGroupTour.html?activeId="+ret.value[i].activeId);
		a6.attr("target","_parent");
		a6.html("查看详情");		
		li.append(a6);
		$("#activeUl").append(li);
		}
...
function unJoinActive(){
	...
	$.post("../../../../activeController/closeActive.do",{"paramter":JSON.stringify({"activeId":activeId})},retJoinActive,"json");
